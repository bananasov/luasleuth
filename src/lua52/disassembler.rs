...
impl<'a> Disassembler<'a> {
    ...
    fn disassemble_debug_info(&self, offset: &mut usize) -> Result<DebugInfo, scroll::Error> {
        let source = LuaString::read(self.buffer, offset, LE)?;
        ...
        let upvalues = self.disassemble_debug_upvalues(offset)?;
    ...
    fn disassemble_debug_upvalues(&self, offset: &mut usize) -> Result<Vec<String>, scroll::Error> {
        let amount: u32 = self.buffer.gread_with(offset, LE)?;
